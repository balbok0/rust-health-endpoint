...
impl Display for HealthStatus {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            HealthStatus::UNHEALTHY(x) => f.write_fmt(format_args!("Unhealthy ({})", x)),
...
impl Default for HealthStatusWithTs {
    fn default() -> Self {
        Self { status: HealthStatus::UNKNOWN, ts: SystemTime::UNIX_EPOCH }
    }
}
...
impl HealthMonitor {
    ...
    pub async fn run(&mut self, signal_receiver: &broadcast::Receiver<()>) {
...
fn signal_receiver_check(signal_receiver: &mut tokio::sync::broadcast::Receiver<()>) -> bool {
    match signal_receiver.try_recv() {
        Ok(()) => {
